...
class FeatureConfig:
    ...
    tolerance_ms: float = 25.0
     ...
     # ===== 修改1.1：Pre-TCN processing switches =====
    pre_tcn_add_mask_channel: bool = False
    pre_tcn_mask_scope: str = "all"  # all|both|audio|video|none
    pre_tcn_add_session_onehot: bool = False
    pre_tcn_qc_gate_video: bool = False
    pre_tcn_vad_split_audio: bool = False
    pre_tcn_add_delta: bool = False
    pre_tcn_add_vad_mouth: bool = False

    pre_tcn_vad_split_groups: list[str] = field(
        default_factory=lambda: ["mel_mfcc"]
    )
    pre_tcn_delta_audio_groups: list[str] = field(
        default_factory=lambda: ["mel_mfcc"]
    )
    pre_tcn_delta_video_groups: list[str] = field(
        default_factory=lambda: ["headpose_geom", "face_behavior", "qc_stats", "vad_agg", "global_motion"]
    )
