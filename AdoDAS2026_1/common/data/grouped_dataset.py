...
def _add_delta_np(x: np.ndarray) -> np.ndarray:
    if x.shape[0] <= 1:
        dx = np.zeros_like(x)
    else:
        dx = np.zeros_like(x)
...
def _repeat_session_onehot(session_idx: int, T: int, n_sessions: int = 4) -> np.ndarray:
    onehot = np.zeros((T, n_sessions), dtype=np.float32)
    onehot[:, session_idx] = 1.0
    return onehot
...
class GroupedParticipantDataset(Dataset):
    ...
    def _mask_scope_includes(self, modality: str) -> bool:
        """Whether pre-TCN per-group mask channel is appended for this modality."""
        if not self.cfg.pre_tcn_add_mask_channel:
            return False
        scope = (self.cfg.pre_tcn_mask_scope or "all").lower()
        if scope in ("none",):
            return False
    ...
    #修改1.4：维度变换函数=============================
    def _post_dim(self, name: str, d: int, modality: str) -> int:
        cfg = self.cfg

        if cfg.pre_tcn_qc_gate_video and modality == "video":
            # gate 不改变维度
            d = d

        if cfg.pre_tcn_add_vad_mouth and modality == "video" and name in {"headpose_geom", "face_behavior"}:
            # 增加 vad*mar 与 silence*mar 两个跨模态通道
            d += 2

        if cfg.pre_tcn_vad_split_audio and modality == "audio" and name in cfg.pre_tcn_vad_split_groups:
            d *= 2

        if cfg.pre_tcn_add_delta:
            if modality == "audio" and name in cfg.pre_tcn_delta_audio_groups:
                d *= 2
            if modality == "video" and name in cfg.pre_tcn_delta_video_groups:
                d *= 2

        if self._mask_scope_includes(modality):
            d += 1

        if cfg.pre_tcn_add_session_onehot:
            d += 4
    ...
    def _probe_dims(self) -> dict[str, int]:
        ...
        '''
        for name, seq in self._load_raw_groups(row, "audio").items():
            dims[name] = self._post_dim(name, seq.features.shape[1], "audio")

        for name, seq in self._load_raw_groups(row, "video").items():
            dims[name] = self._post_dim(name, seq.features.shape[1], "video")
    #==============================
    ...
    def _load_single_session(self, row) -> dict[str, Any] | None:
        ...
        try:
            ...
            '''
            session_idx_tmp = SESSION_TO_IDX.get(str(row["session"]), 0)

            # 先提前取 vad 和 qc，供后面特征增强使用
            vad_signal_np = np.zeros(T, dtype=np.float32)
            if "audio/vad" in aligned_feats:
                v = aligned_feats["audio/vad"]
                vad_signal_np = v[:, 0].astype(np.float32) * aligned_masks["audio/vad"].astype(np.float32)
            elif "video/vad_agg" in aligned_feats:
                v = aligned_feats["video/vad_agg"]
                vad_signal_np = v[:, 0].astype(np.float32) * aligned_masks["video/vad_agg"].astype(np.float32)

            qc_quality_np = np.zeros(T, dtype=np.float32)
            if "video/qc_stats" in aligned_feats:
                v = aligned_feats["video/qc_stats"]
                qc_quality_np = v[:, 0].astype(np.float32) * aligned_masks["video/qc_stats"].astype(np.float32)

            for key, feat in aligned_feats.items():
                modality, name = key.split("/", 1)
                mask = aligned_masks[key].astype(np.float32)
                x = _safe_float_array(feat)

                # 1. video QC 前置门控：只对视频表征类特征做 soft gate
                if cfg.pre_tcn_qc_gate_video and modality == "video" and name not in {"qc_stats", "vad_agg"}:
                    q = qc_quality_np[:, None]
                    x = x * q

                # 2. VAD × mouth/headpose 跨模态联系
                if cfg.pre_tcn_add_vad_mouth and modality == "video" and name in {"headpose_geom", "face_behavior"}:
                    if x.shape[1] >= 5:
                        # headpose_geom: [yaw, pitch, roll, ear_mean, mar]
                        # face_behavior: [ear, mar, yaw, pitch, quality_score]
                        mar_idx = 4 if name == "headpose_geom" else 1
                        mar = x[:, mar_idx:mar_idx + 1]
                        vad = vad_signal_np[:, None]
                        x = np.concatenate([x, vad * mar, (1.0 - vad) * mar], axis=-1)

                # 3. audio speech/silence split
                if cfg.pre_tcn_vad_split_audio and modality == "audio" and name in cfg.pre_tcn_vad_split_groups:
                    vad = vad_signal_np[:, None]
                    x = np.concatenate([x * vad, x * (1.0 - vad)], axis=-1)
                ...
                # 6. session one-hot 提前注入 TCN 前
                if cfg.pre_tcn_add_session_onehot:
                    sess_oh = _repeat_session_onehot(session_idx_tmp, T)
                    x = np.concatenate([x, sess_oh], axis=-1)

                t = torch.from_numpy(x.astype(np.float32))

                if modality == "audio":
                    audio_groups[name] = t
                    audio_mask_parts.append(mask.astype(bool))
                    audio_mask_names.append(name)
                else:
                    video_groups[name] = t
                    video_mask_parts.append(mask.astype(bool))
                    video_mask_names.append(name)
#==================================================
            ...
            #修改1.7：原来后面还有一段计算 vad_signal 和 qc_quality为了避免重复=============================
            #vad_signal = np.zeros(T, dtype=np.float32)
            vad_signal = vad_signal_np
            if "audio/vad" in aligned_feats:
                v = aligned_feats["audio/vad"]
