...
# 替换TCN为TransformerEncoder
'''class TCN(nn.Module):
    def __init__(self, d_model: int, n_layers: int, kernel_size: int, dropout: float) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        # TransformerEncoder的src_key_padding_mask: (B, T), True为padding
        src_key_padding_mask = ~mask.bool()  # 反转，True为padding
