...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--resume_checkpoint", type=str, default=None, help="Path to checkpoint for resuming")
    p.add_argument("--resume_run_name", type=str, default=None, help="Existing run name under output_dir/runs")
    p.add_argument("--resume_latest", type=int, default=None, help="1=resume most recent run that has best.pt")
...
def train_one_epoch_grouped(
    ...
    for batch_idx, batch in enumerate(pbar, start=1):
        ...
        if batch_idx % 20 == 0:
            ...
            elapsed = _fmt_duration(time.time() - epoch_start)
            log.info(
                f"    heartbeat epoch={epoch}/{epochs} step={batch_idx}/{len(loader)} "
                f"loss={loss.item():.4f} avg={avg_loss:.4f} elapsed={elapsed}"
            )
...
def main() -> None:
    ...
    def _resolve_resume_checkpoint() -> tuple[Path | None, str | None]:
        resume_checkpoint_cfg = cfg.get("resume_checkpoint")
        resume_run_name_cfg = cfg.get("resume_run_name")
        resume_latest_cfg = bool(cfg.get("resume_latest", False))

        if resume_checkpoint_cfg:
            ckpt_path = Path(resume_checkpoint_cfg).expanduser().resolve()
            if not ckpt_path.exists():
                raise FileNotFoundError(f"resume_checkpoint not found: {ckpt_path}")
            return ckpt_path, ckpt_path.parent.parent.name

        if resume_run_name_cfg:
            run_dir = output_root / "runs" / str(resume_run_name_cfg)
            ckpt_path = run_dir / "checkpoints" / "best.pt"
            if not ckpt_path.exists():
                raise FileNotFoundError(f"best checkpoint not found for resume_run_name: {ckpt_path}")
            return ckpt_path, run_dir.name

        if resume_latest_cfg:
            runs_root = output_root / "runs"
            candidates = sorted(
                [p for p in runs_root.glob(f"{task}*") if (p / "checkpoints" / "best.pt").exists()],
                key=lambda p: p.stat().st_mtime,
                reverse=True,
            )
            if not candidates:
                raise FileNotFoundError(
                    f"resume_latest=1 but no run with checkpoints found in: {runs_root}"
                )
            latest_run = candidates[0]
            return latest_run / "checkpoints" / "best.pt", latest_run.name

        return None, None

    resume_ckpt_path, resume_run_name = _resolve_resume_checkpoint()
    ...
    '''
    pre_tcn_cfg = cfg.get("pre_tcn", {}) or {}
    ...
    feat_cfg = FeatureConfig(
        ...
        pre_tcn_add_mask_channel=bool(pre_tcn_cfg.get("add_mask_channel", False)),
        pre_tcn_mask_scope=str(pre_tcn_cfg.get("mask_scope", _defaults.pre_tcn_mask_scope)).lower(),
        pre_tcn_add_session_onehot=bool(pre_tcn_cfg.get("add_session_onehot", False)),
        pre_tcn_qc_gate_video=bool(pre_tcn_cfg.get("qc_gate_video", False)),
        pre_tcn_vad_split_audio=bool(pre_tcn_cfg.get("vad_split_audio", False)),
        pre_tcn_add_delta=bool(pre_tcn_cfg.get("add_delta", False)),
        pre_tcn_add_vad_mouth=bool(pre_tcn_cfg.get("add_vad_mouth", False)),

        pre_tcn_vad_split_groups=pre_tcn_cfg.get("vad_split_groups", ["mel_mfcc"]),
        pre_tcn_delta_audio_groups=pre_tcn_cfg.get("delta_audio_groups", ["mel_mfcc"]),
        pre_tcn_delta_video_groups=pre_tcn_cfg.get(
            "delta_video_groups",
            ["headpose_geom", "face_behavior", "qc_stats", "vad_agg", "global_motion"],
        ),
    )
        #==============================
    ...
    log.info(
        f"Pre-TCN mask channel: add={feat_cfg.pre_tcn_add_mask_channel}, "
        f"mask_scope={feat_cfg.pre_tcn_mask_scope}"
    )
    ...
    if device.type == "cuda":
        ...
        n_use = min(2, n_visible)
        if n_use > 1:
            log.info(f"Using DataParallel on {n_use} GPUs")
            grouped_model = nn.DataParallel(grouped_model, device_ids=list(range(n_use)))
            task_head = nn.DataParallel(task_head, device_ids=list(range(n_use)))
    ...
    scaler = None
    if use_amp:
        # Compat: torch>=2.3 has torch.amp.GradScaler, older builds use torch.cuda.amp.GradScaler.
    ...
    if resume_ckpt_path is not None:
        state = load_checkpoint(
            resume_ckpt_path,
            _unwrap_module(grouped_model),
            optimizer=optimizer,
        )
        if "head_state_dict" in state:
            _unwrap_module(task_head).load_state_dict(state["head_state_dict"])
        else:
            log.warning("Resume checkpoint has no head_state_dict; task head starts from current init.")
        ...
        if scheduler_state_dict:
            ...
            for _ in range(resumed_epoch):
                scheduler.step()
        log.info(
            f"Resumed from checkpoint: {resume_ckpt_path} "
            f"(epoch={resumed_epoch}, best_{metric_name}={best_metric:.4f}, next_epoch={start_epoch})"
        )
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if is_best:
            ...
            save_checkpoint(
                ...
                _unwrap_module(grouped_model), optimizer, epoch, best_metric,
                extra={
                    "head_state_dict": _unwrap_module(task_head).state_dict(),
    ...
    state = load_checkpoint(run_dirs["checkpoints"] / "best.pt", _unwrap_module(grouped_model), optimizer=None)
    _unwrap_module(task_head).load_state_dict(state["head_state_dict"])
