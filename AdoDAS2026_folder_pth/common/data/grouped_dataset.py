...
def path_split_for_yaml(cfg: Mapping[str, Any] | None, logical_split: str) -> str | None:
    """Map logical split (train/val) to feature_root subfolder for sequence.npz."""
    if not cfg:
        return None
    flat_keys = {
...
def maybe_default_internal_val_sequence_path_split(cfg: dict[str, Any]) -> str | None:
    """Internal K-fold val.csv participants use feature_root/train on disk."""
    ...
    last = md.name
    looks_internal = (
        "manifests_folder_pth" in md_str
        or "manifests_internal" in md_str
        or (last.startswith("fold_") and "kfold" in md_str)
    )
    if not looks_internal:
        return None
    cfg["val_sequence_path_split"] = "train"
    return (
        "manifest_dir looks like internal K-fold; set val_sequence_path_split=train "
...
class GroupedParticipantDataset(Dataset):
    def __init__(
        ...
        if self.path_split != self.split:
            log.info(
                "GroupedDataset logical split=%s uses feature_root/%s for sequences",
                self.split,
                self.path_split,
            )
        ...
        self._filter_participants_without_features(manifest_path)

        self._feature_dims: dict[str, int] | None = None
        self._cache: list[dict | None] | None = None

    def _sequence_npz_path(self, row, modality: str, feat_name: str) -> Path:
        tag: str | None = None
        if feat_name == "ssl_embed":
            tag = self.cfg.audio_ssl_model_tag
        elif feat_name == "vision_ssl_embed":
            tag = self.cfg.video_ssl_model_tag
        parts = [
            self.root,
            self.path_split,
            str(row["anon_school"]),
        ...
        if tag is not None:
            parts.append(tag)
        parts.append(str(row["session"]))
        return Path(*[str(p) for p in parts]) / "sequence.npz"

    def _session_has_any_feature(self, row) -> bool:
        """Lightweight probe: at least one sequence.npz exists (matches load semantics)."""
        for feat_name in self.cfg.audio_sequence_features:
            if self._sequence_npz_path(row, "audio", feat_name).exists():
                return True
        for feat_name in self.cfg.video_features:
            if self._sequence_npz_path(row, "video", feat_name).exists():
                return True
        return False

    def _count_valid_sessions(self, info: dict[str, Any]) -> int:
        n = 0
        for sess_name in SESSIONS:
            if sess_name in info["sess_rows"] and self._session_has_any_feature(
                info["sess_rows"][sess_name]
            ):
                n += 1
        return n

    def _filter_participants_without_features(self, manifest_path: str | Path) -> None:
        total = len(self.participants)
        kept: list[dict[str, Any]] = []
        dropped: list[tuple[str, str, str]] = []
        for info in self.participants:
            if self._count_valid_sessions(info) > 0:
                kept.append(info)
            else:
                dropped.append((info["anon_school"], info["anon_class"], info["anon_pid"]))
        self.participants = kept
        if not self.participants:
            raise RuntimeError(
                f"No participants with loadable sessions in {manifest_path} "
                f"(split={self.split}, path_split={self.path_split})"
            )
        if dropped:
            preview = ", ".join(f"{s}/{c}/{p}" for s, c, p in dropped[:10])
            more = f" ... (+{len(dropped) - 10} more)" if len(dropped) > 10 else ""
            log.warning(
                f"GroupedDataset split={self.split}: dropped {len(dropped)}/{total} "
                f"participants with no loadable sessions. Examples: {preview}{more}"
            )
...
def _has_any_session(sample: dict[str, Any]) -> bool:
    return any(s is not None for s in sample["sessions"])


def grouped_collate_fn(batch: list[dict[str, Any]]) -> dict[str, Any] | None:
    batch = [s for s in batch if _has_any_session(s)]
    if not batch:
        return None
