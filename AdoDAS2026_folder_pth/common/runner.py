...
from .a1_checkpoint_utils import (
    build_a1_epoch_record,
    save_a1_bias_sidecar,
    update_best_auc_state,
    update_best_raw_f1_state,
    update_best_safe_submit_state,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--early_stop_metric", type=str, default=None,
                    choices=["primary", "val_loss", "safe_submit_f1"],
                    help="Metric for early stopping (A1: safe_submit_f1 = max(raw, shrink0.3, shrink0.5))")
    p.add_argument("--save_last_checkpoint", type=int, default=None,
                    help="1=overwrite checkpoints/last.pt each epoch (optional)")
...
def validate_grouped(
    ...
    if n_batches == 0 or not all_labels:
        raise RuntimeError(
            "validate_grouped: no batches with loadable sessions; "
            "check feature files or run tools/audit_grouped_manifest.py"
        )
    ...
    if task == "a1":
        ...
        from .a1_checkpoint_utils import shrink_f1_from_logits

        shrink03_f1 = shrink_f1_from_logits(logits_np, labels_np, cal_biases, shrink=0.3)
        shrink05_f1 = shrink_f1_from_logits(logits_np, labels_np, cal_biases, shrink=0.5)
        safe_submit_f1 = float(max(mf1, shrink03_f1, shrink05_f1))
        ...
        log.info(
            f"    shrink0.3 F1={shrink03_f1:.4f} shrink0.5 F1={shrink05_f1:.4f} "
            f"safe_submit_f1={safe_submit_f1:.4f}"
        )
        ...
        return {
            ...
            "shrink0.3_f1": shrink03_f1,
            "shrink0.5_f1": shrink05_f1,
            "safe_submit_f1": safe_submit_f1,
            "logits_np": logits_np,
            "labels_np": labels_np,
            "cal_biases_np": cal_biases,
            "primary_metric": mf1,
...
def collect_val_logits_grouped_a1(grouped_model, task_head, loader, device, use_amp,
    ...
    if not all_logits:
        raise RuntimeError("collect_val_logits_grouped_a1: no batches with loadable sessions")
...
def collect_val_logits_grouped_a2(grouped_model, task_head, loader, device, use_amp,
    ...
    for batch in loader:
        ...
        all_labels.append(labels)
    if not all_logits:
        raise RuntimeError("collect_val_logits_grouped_a2: no batches with loadable sessions")
    return np.concatenate(all_logits), np.concatenate(all_labels)
...
def main() -> None:
    ...
    if note_internal:
        log.info(note_internal)

    ckpt_sel = cfg.get("checkpoint_selection", {}) or {}
    if not isinstance(ckpt_sel, dict):
        ckpt_sel = {}
    save_last_checkpoint = bool(
        cfg.get("save_last_checkpoint", ckpt_sel.get("save_last", False))
    )
    ckpt_selection_mode = str(ckpt_sel.get("mode", "safe_submit"))
    ...
    val_ds = GroupedParticipantDataset(
        manifest_dir / "val.csv", feat_cfg, split="val",
    ...
    if task == "a1" and early_stop_metric == "primary":
        early_stop_metric = "safe_submit_f1"
    ...
    if task == "a1":
        log.info(f"A1 checkpoint_selection mode={ckpt_selection_mode}")
    ...
    best_raw_f1_state = {"score": -1.0, "tie_auc": -1.0, "epoch": None}
    best_auc_state = {"score": -1.0, "tie_raw_f1": -1.0, "epoch": None}
    best_safe_submit_state = {"score": -1.0, "tie_auc": -1.0, "tie_raw_f1": -1.0, "epoch": None}
    all_epoch_metrics: list[dict] = []
    a1_selection_rows: list[dict] = []

    log.info("=" * 90)
    if task == "a1":
        log.info(
            "  Epoch  |    LR     | Train Loss | Val Loss | F1 raw | safe_sub |  AUROC | F1[D/A/S]       | Time"
        )
    else:
    ...
    for epoch in range(1, epochs + 1):
        ...
        if task == "a1":
            ...
            safe_sub = float(val_metrics.get("safe_submit_f1", val_metrics["mean_f1"]))
            log.info(
                f"  {epoch:3d}/{epochs:3d} | {lr_now:.2e} |   {train_loss:.4f}   |  {val_metrics['loss']:.4f}  | "
                f"{val_metrics['mean_f1']:.4f} | {safe_sub:.4f} | {val_metrics['auroc']:.4f} | "
        ...
        if task == "a1":
            logits_np = val_metrics.pop("logits_np")
            labels_np = val_metrics.pop("labels_np")
            cal_biases_np = val_metrics.pop("cal_biases_np")
            epoch_rec = build_a1_epoch_record(
                epoch, val_metrics, logits_np, labels_np, cal_biases_np,
            )
            all_epoch_metrics.append(epoch_rec)

            raw_f1 = float(epoch_rec["raw_mean_f1"])
            auroc = float(epoch_rec["macro_auroc"])
            safe_sub = float(epoch_rec["safe_submit_f1"])
            shrink03 = float(epoch_rec["shrink0.3_f1"])
            shrink05 = float(epoch_rec["shrink0.5_f1"])
            cal_biases_list = epoch_rec["full_biases"]

            def _sidecar_for(ckpt_p: Path, ckpt_type: str, ep: int) -> None:
                save_a1_bias_sidecar(
                    ckpt_p,
                    run_name=run_name,
                    epoch=ep,
                    biases=cal_biases_list,
                    checkpoint_type=ckpt_type,
                    raw_mean_f1=raw_f1,
                    macro_auroc=auroc,
                    shrink0_3_f1=shrink03,
                    shrink0_5_f1=shrink05,
                    safe_submit_f1=safe_sub,
                    safe_submit_mode=epoch_rec["safe_submit_mode"],
                    safe_submit_shrink=epoch_rec["safe_submit_shrink"],
                    full_calibrated_mean_f1=float(epoch_rec["full_calibrated_mean_f1"]),
                    full_biases={"D": cal_biases_list[0], "A": cal_biases_list[1], "S": cal_biases_list[2]},
            ...
            if update_best_raw_f1_state(best_raw_f1_state, epoch, raw_f1, auroc):
                ckpt_p = ckpt_root / "best_raw_f1.pt"
                save_checkpoint(ckpt_p, grouped_model, optimizer, epoch, raw_f1, extra=extra_sd)
                _sidecar_for(ckpt_p, "best_raw_f1", epoch)
                log.info(f"  >>> New best_raw_f1={raw_f1:.4f} -> best_raw_f1.pt")
                meta.update_best(epoch, val_metrics)

            if update_best_auc_state(best_auc_state, epoch, auroc, raw_f1):
                ckpt_p = ckpt_root / "best_auc.pt"
                save_checkpoint(ckpt_p, grouped_model, optimizer, epoch, auroc, extra=extra_sd)
                _sidecar_for(ckpt_p, "best_auc", epoch)
                log.info(f"  >>> New best_auc={auroc:.4f} -> best_auc.pt")

            if update_best_safe_submit_state(
                best_safe_submit_state, epoch, safe_sub, auroc, raw_f1,
            ):
                ckpt_p = ckpt_root / "best_safe_submit.pt"
                save_checkpoint(ckpt_p, grouped_model, optimizer, epoch, safe_sub, extra=extra_sd)
                _sidecar_for(ckpt_p, "best_safe_submit", epoch)
                log.info(
                    f"  >>> New best_safe_submit={safe_sub:.4f} "
                    f"mode={epoch_rec['safe_submit_mode']} shrink={epoch_rec['safe_submit_shrink']:.1f} "
                    f"-> best_safe_submit.pt"
                )

            if save_last_checkpoint:
                last_p = ckpt_root / "last.pt"
                save_checkpoint(last_p, grouped_model, optimizer, epoch, raw_f1, extra=extra_sd)
                _sidecar_for(last_p, "last", epoch)
        ...
        if task == "a1":
            ...
            elif early_stop_metric == "safe_submit_f1":
                es_value = float(epoch_rec["safe_submit_f1"])
            else:
                es_value = primary
        else:
    ...
    if task == "a1":
        for name, st, ckpt_file in (
            ("best_raw_f1", best_raw_f1_state, "best_raw_f1.pt"),
            ("best_auc", best_auc_state, "best_auc.pt"),
        ):
            ep = st.get("epoch")
            if ep is None:
                continue
            rec = next((m for m in all_epoch_metrics if m["epoch"] == ep), None)
            a1_selection_rows.append({
                "checkpoint_name": ckpt_file,
                "epoch": ep,
                "raw_mean_f1": st["score"] if name == "best_raw_f1" else (rec or {}).get("raw_mean_f1", ""),
                "macro_auroc": st["tie_auc"] if name == "best_raw_f1" else st["score"],
                "full_calibrated_mean_f1": (rec or {}).get("full_calibrated_mean_f1", ""),
                "reference_f1_raw": (rec or {}).get("raw_mean_f1", ""),
                "recommended_default_submission": False,
            })
        ep = best_safe_submit_state.get("epoch")
        if ep is not None:
            rec = next((m for m in all_epoch_metrics if m["epoch"] == ep), {})
            a1_selection_rows.append({
                "checkpoint_name": "best_safe_submit.pt",
                "epoch": ep,
                "raw_mean_f1": rec.get("raw_mean_f1", ""),
                "macro_auroc": rec.get("macro_auroc", ""),
                "safe_submit_f1": rec.get("safe_submit_f1", ""),
                "safe_submit_mode": rec.get("safe_submit_mode", ""),
                "safe_submit_shrink": rec.get("safe_submit_shrink", ""),
                "full_calibrated_mean_f1": rec.get("full_calibrated_mean_f1", ""),
                "reference_f1_raw": rec.get("raw_mean_f1", ""),
                "recommended_default_submission": True,
            })
        selections = {}
        for row in a1_selection_rows:
            selections[row["checkpoint_name"]] = row
        write_checkpoint_meta(
            ckpt_dir,
            run_name=run_name,
        ...
        write_selection_summary(run_dirs["root"], a1_selection_rows)
        load_ckpt = ckpt_dir / "best_safe_submit.pt"
        if not load_ckpt.exists():
            load_ckpt = ckpt_dir / "best_raw_f1.pt"
        if not load_ckpt.exists():
            load_ckpt = ckpt_dir / "best_auc.pt"
        log.info(f"Loading {load_ckpt.name} for post-train calibration / optional submission ...")
    else:
        load_ckpt = ckpt_dir / "best.pt"
        log.info("Loading best.pt for calibration / submission generation ...")

    state = load_checkpoint(load_ckpt, grouped_model, optimizer=None)
    ...
    if task == "a1":
        ...
        a1_biases = biases
        meta.set_extra("final_selected_strategy", "raw_for_submission")
        meta.set_extra("final_selected_metrics", {
            ...
            "note": "full_calibrated_f1 is analysis-only; default submission uses raw + matched sidecar",
        })
        ...
        log.info(
            ...
            "infer defaults to --a1_bias_mode none unless sidecar/auto is requested."
        )
    else:
    ...
    if bool(cfg.get("run_inference_after_train", False)):
        ...
        for split_name in ("val", "test_hidden"):
            ...
            ds = GroupedParticipantDataset(
                manifest_path, feat_cfg, split=split_name,
