...
class BackboneConfig:
    ...
    # SafeNorm 模型侧：低维行为特征不做 per-frame LayerNorm，避免抹掉物理量（如 LayerNorm(1) 对 vad）。
    low_dim_no_layernorm: bool = True
    low_dim_threshold: int = 16
    force_no_layernorm_groups: tuple[str, ...] = ()
    force_layernorm_groups: tuple[str, ...] = ()
...
def _adapter_use_layernorm(group_name: str, d_in: int, cfg: BackboneConfig) -> bool:
    """是否对该组使用 LayerNorm；强制名单优先于维度阈值。"""
    if cfg.force_layernorm_groups and group_name in cfg.force_layernorm_groups:
        return True
    if cfg.force_no_layernorm_groups and group_name in cfg.force_no_layernorm_groups:
        return False
    if not cfg.low_dim_no_layernorm:
        return True
    return d_in > cfg.low_dim_threshold


class GroupAdapter(nn.Module):
    """
    TCN 前每组特征的投影适配器。
    高维 embedding 保留 LayerNorm 以稳定尺度；低维或强制名单组用 Identity，
    与数据侧 train mean/std 归一化配合，避免 per-frame LN 破坏 yaw/vad/qc 等物理含义。
    """
    ...
    def __init__(
        ...
        use_ln = _adapter_use_layernorm(group_name, d_in, backbone_cfg)
        self.norm: nn.Module = nn.LayerNorm(d_in) if use_ln else nn.Identity()
...
class _PooledGroupAdapter(nn.Module):
    """与 GroupAdapter 相同策略，用于 session 级 pooled 向量（如 egemaps）。"""
    ...
    def __init__(
        ...
        self.net = nn.Sequential(
            nn.LayerNorm(d_in) if use_ln else nn.Identity(),
...
class MTCNBackbone(nn.Module):
    def __init__(self, cfg: BackboneConfig) -> None:
        ...
        self.audio_adapters = nn.ModuleDict({
            name: GroupAdapter(d_in, cfg.d_adapter, cfg.dropout, name, cfg)
            for name, d_in in cfg.audio_group_dims.items()
        })
        self.audio_pooled_adapters = nn.ModuleDict({
            name: _PooledGroupAdapter(d_in, cfg.d_adapter, cfg.dropout, name, cfg)
