...
from .data.dataset import (
    ...
    ITEM_COLS,
    FeatureConfig,
    build_feature_config_from_run_cfg,
)
...
def backbone_safenorm_kwargs(cfg: dict) -> dict:
    """从 YAML pre_tcn_safenorm 解析 MTCN Backbone 的低维 LayerNorm 策略。"""
    sn = cfg.get("pre_tcn_safenorm") or {}
    if not bool(sn.get("enabled", False)):
        # 与改造前一致：每组都使用 LayerNorm，不读 force 名单。
        return {
            "low_dim_no_layernorm": False,
            "low_dim_threshold": 16,
            "force_no_layernorm_groups": (),
            "force_layernorm_groups": (),
        }
    fn = sn.get("force_no_layernorm_groups") or ()
    fl = sn.get("force_layernorm_groups") or ()
    if isinstance(fn, list):
        fn = tuple(fn)
    elif not isinstance(fn, tuple):
        fn = ()
    if isinstance(fl, list):
        ...
        fl = ()
    return {
        "low_dim_no_layernorm": bool(sn.get("low_dim_no_layernorm", True)),
        "low_dim_threshold": int(sn.get("low_dim_threshold", 16)),
        "force_no_layernorm_groups": fn,
        "force_layernorm_groups": fl,
    }
...
def validate_grouped(
    ...
    if task == "a1":
        ...
        log.info(
            f"    pred dist: 0={pred_pct[0]:.1f}% 1={pred_pct[1]:.1f}% 2={pred_pct[2]:.1f}% 3={pred_pct[3]:.1f}%"
        )
...
def main() -> None:
    ...
    feat_cfg = build_feature_config_from_run_cfg(cfg)
    if rank == 0:
        log.info(f"Mask policy: {feat_cfg.mask_policy}")
    sn = cfg.get("pre_tcn_safenorm") or {}
    if rank == 0:
        if feat_cfg.safenorm_enabled:
            log.info(
                "SafeNorm: enabled apply_stats=%s stats_path=%s clip=%s",
                feat_cfg.safenorm_apply_stats,
                feat_cfg.safenorm_stats_path,
                feat_cfg.safenorm_clip_value,
            )
        elif sn:
            log.info("pre_tcn_safenorm present in YAML but enabled=false (baseline path)")
