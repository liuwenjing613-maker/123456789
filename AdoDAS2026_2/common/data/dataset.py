...
from .safenorm_stats import apply_mean_std_clip, load_stats_npz
...
class FeatureConfig:
    ...
    # --- SafeNorm（仅 TCN 前）：训练集 mean/std + clip 在数据侧；低维跳过 LN 在 BackboneConfig。
    safenorm_enabled: bool = False
    safenorm_apply_stats: bool = True
    safenorm_stats_path: str | None = None
    safenorm_clip_value: float = 5.0
    safenorm_eps: float = 1e-6
    safenorm_norm_type: str = "mean_std"
...
def build_feature_config_from_run_cfg(cfg: dict) -> FeatureConfig:
    """由 train/infer 的扁平 cfg（已合并 feature_selection）构造 FeatureConfig。"""
    d = FeatureConfig()
    sn = cfg.get("pre_tcn_safenorm") or {}
    return FeatureConfig(
        feature_root=cfg.get("feature_root", d.feature_root),
        audio_features=cfg.get("audio_features", d.audio_features),
        video_features=cfg.get("video_features", d.video_features),
        audio_ssl_model_tag=cfg.get("audio_ssl_model_tag", d.audio_ssl_model_tag),
        video_ssl_model_tag=cfg.get("video_ssl_model_tag", d.video_ssl_model_tag),
        grid_step_ms=float(cfg.get("grid_step_ms", d.grid_step_ms)),
        tolerance_ms=float(cfg.get("tolerance_ms", d.tolerance_ms)),
        mask_policy=cfg.get("mask_policy", d.mask_policy),
        core_audio=cfg.get("core_audio", d.core_audio),
        core_video=cfg.get("core_video", d.core_video),
        safenorm_enabled=bool(sn.get("enabled", False)),
        safenorm_apply_stats=bool(sn.get("apply_stats", True)),
        safenorm_stats_path=sn.get("stats_path"),
        safenorm_clip_value=float(sn.get("clip_value", 5.0)),
        safenorm_eps=float(sn.get("eps", 1e-6)),
        safenorm_norm_type=str(sn.get("norm_type", "mean_std")),
    )


def safenorm_stats_for_cfg(cfg: FeatureConfig) -> dict[str, dict[str, np.ndarray]]:
    """训练/验证/推理共用：仅当开启且需 apply 时加载 npz。"""
    if (
        not cfg.safenorm_enabled
        or not cfg.safenorm_apply_stats
        or not cfg.safenorm_stats_path
    ):
        if (
            cfg.safenorm_enabled
            and cfg.safenorm_apply_stats
            and not cfg.safenorm_stats_path
        ):
            log.warning(
                "SafeNorm: apply_stats 已开启但未配置 stats_path，跳过训练集归一化"
            )
        return {}
    if cfg.safenorm_norm_type != "mean_std":
        log.warning(
            "SafeNorm norm_type=%r not implemented; using loaded mean/std only",
            cfg.safenorm_norm_type,
        )
    return load_stats_npz(cfg.safenorm_stats_path)["stats"]
...
class MultimodalDataset(Dataset):
    ...
    def __init__(
        ...
        self._safenorm_stats: dict[str, dict[str, np.ndarray]] = safenorm_stats_for_cfg(cfg)
    ...
    def _load_sample(self, idx: int) -> dict[str, Any]:
        ...
        for key, feat in aligned_feats.items():
            ...
            # ASP 的 vad/qc 仍用下方原始 aligned 量；此处仅对送入 GroupAdapter 的张量做 train 统计归一化。
            if self._safenorm_stats:
                arr = apply_mean_std_clip(
                    arr,
                    name,
                    self._safenorm_stats,
                    eps=self.cfg.safenorm_eps,
                    clip_value=self.cfg.safenorm_clip_value,
                )
        ...
        if "egemaps" in cfg.audio_pooled_features:
            ...
            if egemaps is not None:
                eg_arr = egemaps.astype(np.float32)
                if self._safenorm_stats:
                    eg_arr = apply_mean_std_clip(
                        eg_arr,
                        "egemaps",
                        self._safenorm_stats,
                        eps=self.cfg.safenorm_eps,
                ...
                audio_pooled_groups["egemaps"] = torch.from_numpy(eg_arr)
            else:
                audio_pooled_groups["egemaps"] = torch.zeros(dims.get("egemaps", 88))
