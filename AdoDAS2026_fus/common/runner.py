...
from .gpu_argv import apply_remove_gpu_arg
...
from .models.heads import A1Head, A2OrdinalHead, a1_loss, a2_ordinal_loss  # , cross_modal_consistency_loss
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--cross_modal_consistency_weight", type=float, default=None, help="Weight for cross-modal consistency loss")
...
def broadcast_val_metrics_for_ddp(
    task: str,
    ...
    """
    用 NCCL broadcast 标量，避免 broadcast_object_list 对含 numpy 的大 dict 做 pickle
    （部分环境下易在训练后期/退出时 SIGSEGV）。
    rank0 返回完整 val_metrics；其它 rank 返回仅含 early_stop / is_best 所需字段的最小 dict。
    """
    buf = torch.zeros(_VAL_MET_BUF, dtype=torch.float64, device=device)
    if rank == 0:
        assert val_metrics is not None
        buf[0] = float(val_metrics["loss"])
        buf[1] = float(val_metrics["primary_metric"])
        if task == "a1":
            buf[2] = float(val_metrics.get("mean_f1", 0.0))
            buf[3] = float(val_metrics.get("auroc", 0.0))
            buf[4] = float(val_metrics.get("mean_f1_calibrated", val_metrics.get("mean_f1", 0.0)))
        else:
            buf[2] = float(val_metrics.get("mean_qwk", 0.0))
            buf[3] = float(val_metrics.get("mean_mae", 0.0))
            buf[4] = 0.0
    dist.broadcast(buf, src=0)
    if rank == 0:
        return val_metrics  # type: ignore[return-value]
    if task == "a1":
        return {
            "loss": buf[0].item(),
            "primary_metric": buf[1].item(),
            "mean_f1": buf[2].item(),
            "auroc": buf[3].item(),
            "mean_f1_calibrated": buf[4].item(),
            "pcf1": np.zeros(3, dtype=np.float32),
            "pcf1_calibrated": np.zeros(3, dtype=np.float32),
            "calibration_biases": [0.0, 0.0, 0.0],
            "selection_source": "ddp_scalar_sync",
        }
    return {
        "loss": buf[0].item(),
        "primary_metric": buf[1].item(),
        "mean_qwk": buf[2].item(),
        "mean_mae": buf[3].item(),
        "selected_decode_method": None,
    }


_RUN_NAME_MAX_PAYLOAD = 2048


def broadcast_run_name_for_ddp(
    run_name: str | None,
    rank: int,
    device: torch.device,
) -> str:
    """
    从 rank0 同步 run_name：固定形状 uint8 + dist.broadcast。
    避免启动阶段 broadcast_object_list（部分 NCCL 环境下数秒内 SIGSEGV）。
    """
    buf = torch.zeros(4 + _RUN_NAME_MAX_PAYLOAD, dtype=torch.uint8, device=device)
    if rank == 0:
        assert run_name is not None
        raw = run_name.encode("utf-8")
        if len(raw) > _RUN_NAME_MAX_PAYLOAD:
            raise RuntimeError(
                f"run_name UTF-8 length {len(raw)} > {_RUN_NAME_MAX_PAYLOAD}; shorten config tags"
            )
        L = len(raw)
        buf[0] = L & 0xFF
        buf[1] = (L >> 8) & 0xFF
        ...
        buf[3] = (L >> 24) & 0xFF
        if L:
            buf[4 : 4 + L] = torch.tensor(bytearray(raw), dtype=torch.uint8, device=device)
    dist.broadcast(buf, src=0)
    L = (
        int(buf[0].item())
        | (int(buf[1].item()) << 8)
    ...
    if L < 0 or L > _RUN_NAME_MAX_PAYLOAD:
        raise RuntimeError(f"invalid DDP run_name length {L}")
    if L == 0:
        return ""
    return buf[4 : 4 + L].detach().cpu().numpy().tobytes().decode("utf-8")


def _ddp_teardown_safe(device: torch.device, use_ddp: bool) -> None:
    """同步 CUDA、释放大引用后再销毁进程组，降低退出阶段 SIGSEGV 概率。"""
...
def _distributed_context() -> tuple[bool, int, int, int, torch.device]:
    """
    torchrun 多卡: WORLD_SIZE>1 时初始化 NCCL，返回 (is_ddp, rank, local_rank, world_size, device)。
    单进程训练: is_ddp=False，device=cuda:0 或 cpu。
    """
    ...
    # 单机多卡上，部分环境会误走 IB/verbs 并在首次 collective 时 SIGSEGV；
    # 默认禁用 NCCL InfiniBand（多机 IB 训练请在 shell 中 export NCCL_IB_DISABLE=0）。
    ...
    return True, rank, local_rank, ws, torch.device(f"cuda:{local_rank}")


def _silence_non_primary_logging() -> None:
    root = logging.getLogger()
...
def train_one_epoch_grouped(
    ...
    cross_modal_consistency_weight: float = 0.0,
    *,
    ddp: bool = False,
    ddp_rank: int = 0,
) -> float:
    ...
    for batch in iterator:
        ...
        with torch.amp.autocast("cuda", enabled=use_amp, dtype=torch.bfloat16):
            ...
            # Add cross-modal consistency loss if weight > 0
            # if cross_modal_consistency_weight > 0 and "audio_features" in out and "video_features" in out:
            #     consistency_loss = cross_modal_consistency_loss(
            #         out["audio_features"], out["video_features"],
            #         out["audio_mask"], out["video_mask"]
            #     )
            #     loss = loss + cross_modal_consistency_weight * consistency_loss
    ...
    if ddp and dist.is_initialized():
        t = torch.tensor([total_loss, float(n_batches)], device=device, dtype=torch.float64)
        dist.all_reduce(t, op=dist.ReduceOp.SUM)
        avg = (t[0] / max(t[1], 1.0)).item()
    return avg
...
def main() -> None:
    ...
    use_ddp, rank, local_rank, world_size, device = _distributed_context()
    ...
    if use_ddp:
        if rank == 0:
            run_name = build_run_name(
                ...
                training_mode="grouped_participant",
            )
        else:
            run_name = None
        run_name = broadcast_run_name_for_ddp(run_name, rank, device)
    else:
        run_name = build_run_name(
            cfg, task, datetime.now().strftime("%Y%m%d_%H%M%S"), training_mode="grouped_participant"
        )
    ...
    if rank == 0:
        ...
        _silence_non_primary_logging()

    if rank == 0:
        vis = os.environ.get("CUDA_VISIBLE_DEVICES")
        if vis is None or str(vis).strip() == "":
            log.warning(
                "CUDA_VISIBLE_DEVICES 未设置：本进程内 PyTorch 的 cuda:0 对应 **物理 GPU 0**。"
                " 若要用其它卡，请在命令前加 export CUDA_VISIBLE_DEVICES=1,6 或使用 "
                "python train.py --gpu 1,6 ...（torchrun 时同样先 export 或把 --gpu 紧跟在 train.py 之前由 shell 设置）。"
            )
        else:
            log.info(
                "CUDA_VISIBLE_DEVICES=%s — 本进程内 cuda:0 = 可见列表中的第 1 张物理卡；"
                "PyTorch OOM 里的「GPU 0」是此逻辑编号，不是机器上的 PCI 0 号。",
                vis,
            )
        log.info("Device: %s (rank=%s local_rank=%s world_size=%s)", device, rank, local_rank, world_size)
        if use_ddp:
            log.info(
                "DDP: 配置中的 batch_size 表示全局 participant batch，将均分到各 GPU。"
                " 例如 batch_size=64、2 卡 -> 每卡 32。"
            )
        log.info(f"Task: {task}")
        log.info(f"Run name: {run_name}")
        log.info(f"Config: {cfg}")

    meta = RunMetadata(run_dirs["root"], cfg, task, run_name) if rank == 0 else None
    ...
    feat_cfg = FeatureConfig(
        ...
        pre_tcn_processing=cfg.get("pre_tcn_processing", _defaults.pre_tcn_processing),
    )
    if rank == 0:
    ...
    batch_size_global = int(cfg.get("batch_size", 64))
    if use_ddp:
        if batch_size_global % world_size != 0:
            raise ValueError(
                f"batch_size={batch_size_global} 必须能被 GPU 数 {world_size} 整除（DDP 按卡均分 participant batch）。"
            )
        train_batch = batch_size_global // world_size
    else:
        train_batch = batch_size_global
    val_batch = batch_size_global

    if use_ddp:
        n_train = len(train_ds)
        samples_per_rank = int(math.ceil(n_train / float(world_size)))
        if train_batch > samples_per_rank:
            new_tb = max(1, samples_per_rank)
            if rank == 0:
                log.warning(
                    "DDP: per-GPU train_batch=%d 大于每 rank 可用样本数 ceil(%d/%d)=%d，"
                    "自动降为 %d；当前全局等效 participant batch=%d（可在配置中显式改小 batch_size）。",
                    train_batch,
                    n_train,
                    world_size,
                    samples_per_rank,
    ...
    if preload:
        if rank == 0:
            ...
            if use_ddp:
                log.warning(
                    "DDP 下每个进程都会 preload 一份数据，主机内存约为单卡的 %.0fx；"
                    "内存不足请设 preload: false。",
                    world_size,
                )
        t_pre = time.time()
        ...
        if rank == 0:
            log.info(
                f"Preload done: {train_gb:.1f}G + {val_gb:.1f}G = {train_gb + val_gb:.1f}G, "
    ...
    if rank == 0:
        log.info(
            "batch_size(global)=%s per_gpu_train=%s val_batch=%s num_workers=%s",
            batch_size_global,
            train_batch,
            val_batch,
            num_workers,
    ...
    bb_cfg = BackboneConfig(
        ...
        post_tcn_cross_attention=bool(cfg.get("post_tcn_cross_attention", True)),
    )
    ...
    n_params = sum(p.numel() for p in _unwrap_model(grouped_model).parameters()) + sum(
        p.numel() for p in _unwrap_model(task_head).parameters()
    )
    if rank == 0:
        log.info(f"Model params: {n_params:,}")
        log.info("Post-TCN cross attention: %s", bb_cfg.post_tcn_cross_attention)
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss = train_one_epoch_grouped(
            ...
            cross_modal_consistency_weight=cfg.get("cross_modal_consistency_weight", 0.0),
            ddp=use_ddp,
            ddp_rank=rank,
        )
        ...
        if use_ddp:
            if rank == 0:
                ...
                val_metrics = None
            val_metrics = broadcast_val_metrics_for_ddp(task, val_metrics, device, rank)
        else:
            val_metrics = validate_grouped(
                grouped_model, task_head, val_loader, device,
                task, epoch, epochs, use_amp, pos_weight=pos_weight_t,
        ...
        if is_best:
            ...
            if rank == 0:
                save_checkpoint(
                    ...
                    _unwrap_model(grouped_model),
                    optimizer,
                    epoch,
                    best_metric,
                    extra={"head_state_dict": _unwrap_model(task_head).state_dict()},
                )
        ...
        if early_stop.step(es_value):
            if rank == 0:
                log.info(
                    f"  EarlyStopping triggered at epoch {epoch} (patience={patience}, metric={early_stop_metric})"
                )
            break
    ...
    if rank == 0:
        ...
        gm_u = _unwrap_model(grouped_model)
        th_u = _unwrap_model(task_head)
        state = load_checkpoint(run_dirs["checkpoints"] / "best.pt", gm_u, optimizer=None)
        th_u.load_state_dict(state["head_state_dict"])
        gm_u.to(device)
        th_u.to(device)
        ...
        if task == "a1":
            ...
            val_logits, val_labels = collect_val_logits_grouped_a1(
                gm_u, th_u, val_loader, device, use_amp,
    ...
    try:
        if use_ddp:
            try:
                del grouped_model, task_head, optimizer, train_loader, val_loader, train_sampler
            except NameError:
                pass
            try:
                del scaler
        ...
        _ddp_teardown_safe(device, use_ddp)
