...
class BackboneConfig:
    ...
    d_session: int = 16
    d_shared: int = 256
    post_tcn_cross_attention: bool = True
...
class MTCNBackbone(nn.Module):
    def __init__(self, cfg: BackboneConfig) -> None:
        ...
        self.post_tcn_cross_attention = bool(cfg.post_tcn_cross_attention)
        if self.post_tcn_cross_attention:
            # AdoDAS2026-1 style post-TCN cross attention branch.
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.post_tcn_cross_attention:
            # AdoDAS2026-1 style post-TCN fusion.
        ...
        # Keep a stable output interface for GroupedModel and optional consistency losses.
        return {
            "output": output,
